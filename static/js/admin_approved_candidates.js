  ...
  fetchWithRetry("/admin/api/get-approved-candidates", 5)
    .then((resp) => {
      ...
      if (resp.success) {
        ...
        grid.innerHTML = `<div class="empty-card"><div class="empty-card-icon">⚠️</div><p>Error: ${
          resp.error || "Unknown"
        }</p></div>`;
...
function renderApprovedCandidates(container, countElem, rows) {
  ...
  rows.forEach((r) => {
    ...
    const idProofBtn = r.id_proof_name
      ? `<button class="table-action-btn table-view-btn" onclick="viewApprovedFile('${usn}', 'id_proof')">View ID Proof</button>`
      : "—";
    const resumeBtn = r.resume_name
      ? `<button class="table-action-btn table-view-btn" onclick="viewApprovedFile('${usn}', 'resume')">View Resume</button>`
      : "—";
    const projectBtn = r.project_document_name
      ? `<button class="table-action-btn table-view-btn" onclick="viewApprovedFile('${usn}', 'project')">View Project</button>`
      : "—";
  ...
  fetch("/admin/api/get-rejection-reasons")
    ...
    .then((data) => {
      if (data.success) {
        ...
        document.getElementById("approvedRejectionModal").style.display =
          "flex";
      }
    })
